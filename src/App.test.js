...
test('renders please wait whilst fetching the api data', () => {
  ...
  const pleaseWait = getByText(
    /... Please wait while the exercise list is populated!/i
  );
...
test('correctly renders the data from the api on a succesful call', async () => {
  ...
  const { getByText } = render(<App />);
  const bodyAreaText = await waitForElement(() =>
    getByText(
      /Grab the pull up bar, hands facing forwards. Retract your shoulder blades and tilt your torso back./i
    )
  );
